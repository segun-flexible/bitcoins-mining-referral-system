...
userDashBoard.get("/profile/:id", isCurrentUser, async (req, res) => {
    ...
    if (req.id) {
        const currentUser = await getSomeProfileInfo(req.id);
        const referralList = await getAllReferral(req.id);
        
        try {
            const viewingProfile = await getSomeProfileInfo(req.params.id);

            //user profile found
            if (viewingProfile.id) {
                return res.render("userdashboard/profile", {
                        title: `${viewingProfile.username} 's Profile`,
                        user:currentUser,
                        url: process.env.URL,
                        referralList,
            ...
            //user profile not found
               ...
               return res.render('mics/profile404') 
            }
        }
